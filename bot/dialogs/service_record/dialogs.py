...
from .getters import (getter_select_type,
                      ...
                      getter_service_part_edit_menu,
                      getter_service_part_edit_params)
from .handlers import (select_service_type,
                       ...
                       add_work_button,
                       enter_service_part_name,
                       select_service_part_edit_param,
                       enter_service_part_edit_param,
                       add_part_button,
                       delete_service_part_button,
                       save_button)
...
service_part_dialog = Dialog(
    ...
    Window(
        ...
        Group(Select(Format("{item[0]}"),
                     ...
                     item_id_getter=lambda x: x[1],
                     items="buttons",
                     on_click=select_service_part_edit_param),
              Button(Format("{delete_service_part_button}"),
                     id="delete_service_part_button",
                     on_click=delete_service_part_button),
              width=2),
        Group(Select(Format("{item[0]}"),
                     id="paginator_buttons",
                     item_id_getter=lambda x: x[1],
                     ...
                     on_click=paginator_handler),
              width=3),
        Button(Format("{add_part_button}"),
               id="add_part_button",
               on_click=add_part_button),
        Button(Format("{back_button}"),
    ...
    Window(
        Format("{service_part_edit_text}"),
        MessageInput(func=enter_service_part_edit_param,
                     content_types=ContentType.TEXT),
        MessageInput(func=error_enter_no_text),
        Button(Format("{back_button}"),
               id="back_button_to_edit_service_part_param",
               on_click=back_button),
        getter=getter_service_part_edit_params,
        state=ServicePartState.param_edit_text
    )
)
